...
- PostgreSQL (Supabase)
- Judge: JavaScript vía proceso Node.js (`judge/run.mjs`, sandbox `node:vm` + timeout); Java vía `javac`/`java` directo
- Mailpit (SMTP de desarrollo con UI web)

## Setup

Requisitos: PHP 8.4+, Composer, Node.js, JDK (judge de Java), una base Postgres (Supabase).
...
- **`java`**: el código debe ser una clase `public class Main` con `main(String[] args)` que lee de `System.in` (típicamente con `Scanner`) e imprime con `System.out.println`. `JudgeSubmission` compila con `javac` y corre con `java` directo en el worker (sin sandbox de contenedor) — se evaluó self-hostear Piston ([engineer-man/piston](https://github.com/engineer-man/piston)), pero su sandbox `isolate` necesita delegación de cgroup v2 sobre `/sys/fs/cgroup`, que Railway no otorga a ningún contenedor (mismo problema de fondo que tuvo Judge0 con `isolate`, ver nota de troubleshooting). Mismo nivel de confianza "aula" que el sandbox de JS: no es defensa contra código adversarial.
